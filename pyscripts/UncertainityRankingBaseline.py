...
class UncertainityRankingBaseline:
    def __init__(self):
    ...
    def run(self,prbCrFile,prbCpFile,wd,trf,tpf,rJf,pJf,totalTestInstances,cMatrix):
        ...
        for docID,probVal in prCrRanked.items():
            if not Tau_r==0:
                ...
                Tau_r=Tau_r-1
        cPickle.dump(docids2Annotatep2,open(wd+'/docs4map2URBaseline.list.'+str(totalTestInstances)+'.p', 'wb'))
        p2=phase2()
        p2.cmv=cMatrix
        p2.runphase2(prbCrFile,prbCpFile,rJf,wd,totalTestInstances,self.get_tauR(trf),BaselineType='UR')
        ...
        cPickle.dump(docids2Annotatep3,open(wd+'/docs4map3URBaseline.list.'+str(totalTestInstances)+'.p', 'wb'))
        p3=phase3()
        p3.cmv=cMatrix
        p3.runphase3(prbCpFile,pJf,wd,totalTestInstances,self.get_tauP(tpf),BaselineType='UR')
        ...
        print "Ending Uncertainity Ranking Baseline Model "
    ...
    def ComputeMisclassificationCost(self,wd,rJudgmentsFile,pJudgmentsFile,totalTestInstances,cMatrix):
        ...
        cP=cPickle.load(open(wd+"/UR.docid-D_P-risk.dictionary.p3."+str(totalTestInstances)+".p",'rb'))
        cL=cPickle.load(open(wd+"/UR.docid-D_L-risk.dictionary.p3."+str(totalTestInstances)+".p",'rb'))
        cW=cPickle.load(open(wd+"/UR.docid-D_W-risk.dictionary.p3."+str(totalTestInstances)+".p",'rb'))
        ...
        print "Uncertainity Ranking Controlled Condition"
...
def main(argv):
    try:
        urbaseline=UncertainityRankingBaseline()
        responsiveWD=argv[0]
        privilegeWD=argv[1]
        wd=argv[2]
        rJf=argv[3]
        ...
        print "Starting Uncertainity Ranking Baseline Model "
        cMatrix=cm()
        ...
        urbaseline.run(responsiveWD+'/pickleFiles/ds-op-label.tuple.dictionary.'+str(numTestInstances)+'.p',privilegeWD+'/pickleFiles/ds-op-label.tuple.dictionary.'+str(numTestInstances)+'.p',wd, wd+'/tauR_value.'+str(numTestInstances)+'.p',wd+'/tauP_value.'+str(numTestInstances)+'.p',rJf,pJf,numTestInstances,cmv)
    except:
        raise
